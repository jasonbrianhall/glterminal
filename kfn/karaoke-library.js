...
class KaraokePlayer {
    ...
    parseINI(content) {
        ...
        for (let line of lines) {
            ...
            if (currentSection === 'general' || currentSection === '') {
                ...
                if (key.toLowerCase() === 'artist') artist = value;
            }
    ...
    createAudioElement(url, filename, index) {
        ...
        const isVocal = !filename.toLowerCase().includes('instru') && !filename.toLowerCase().includes('beat');
        ...
        if (isVocal && !this.audioVocal) {
            this.audioVocal = audio;
        } else if (!isVocal && !this.audioBacking) {
