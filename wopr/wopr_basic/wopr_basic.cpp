...
// blocking=true: wait for a key (GET); blocking=false: return "" if no key
// ready (INKEY$)
...
// Evaluate a string expression: string literal or string variable or string
// function
string str_expression(void) {
  string s;
  if (toktype == kSTRING) {
...
void datastmt(
    void) { // DATA val, val, ... — scan program for all DATA lines at run time
...
void collect_data(void) {
  ...
  for (int i = 1; i < c_maxlines; ++i) {
    ...
    if (dp == string::npos)
      continue;
    // make sure it's a keyword (preceded by space or start, followed by
    // space/digit/sign)
...
void dimstmt(
    void) { // DIM @(n) — resize the single array; other DIM forms are no-ops
